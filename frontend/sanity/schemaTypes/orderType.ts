...
export const orderType = defineType({
    ...
    preview: {
        ...
        prepare(selection) {
            const orderIdSnipet = `${selection.orderId.slice(0,5)}...${selection.orderId.slice(selection.orderId.length-5,selection.orderId.length)}`
            return {
                title: `${selection.name} (${orderIdSnipet})`,
